...
const LoginForm: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await login(email, senha); // ✅ CORREÇÃO: Passando email e senha como argumentos separados
            navigate('/dashboard');
        } catch (err: unknown) { // ✅ CORREÇÃO: Tipagem de erro para unknown
            console.error('Erro no login:', err);

            const axiosError = err as { // ✅ CORREÇÃO: Cast para tipo conhecido do Axios
                response?: {
                    status?: number;
                    data?: { message?: string };
                };
            ...
            if (axiosError.response) {
                if (axiosError.response.status === 401) {
                    ...
                    setError(axiosError.response.data?.message || 'Erro ao fazer login. Tente novamente.');
                }
